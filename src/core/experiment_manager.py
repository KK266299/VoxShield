...
class ExperimentManager:
    ...
    def build_noise_backend(self, keys):
        ...
        tied_channels = bool(get_config(params, "tied_channels", True))

        if self.train_loader is None:
            raise RuntimeError("[ExperimentManager] train_loader is None; cannot infer image_size for UE backend")

        ds = self.train_loader.dataset
        if isinstance(ds, ConcatDataset):
            base_ds = ds.datasets[0]
        else:
            base_ds = ds
        ...
        if isinstance(sample, dict) and "image" in sample:
            ...
            raise RuntimeError("[ExperimentManager] Expect dataset[i] to be dict with key 'image' for UE backend")

        if isinstance(img, torch.Tensor):
            vol_shape = tuple(img.shape)
        else:
            import numpy as np
        ...
        self.logger.info(
            ...
            f"epsilon={epsilon}, tied_channels={tied_channels}, image_size={image_size}"
        )
    ...
    def setup_trainer(self) -> None:
        ...
        if "seg" in self.task_name.lower():
